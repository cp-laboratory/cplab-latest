...
import { Cpu, Mail, MapPin, Phone, ExternalLink } from "lucide-react";
...
const footerLinks = {
  ...
  research: [
    { label: "Application Development", href: "/projects" },
    { label: "Machine Learning", href: "/projects" },
    { label: "Blockchain", href: "/projects" },
    { label: "IoT", href: "/projects" },
    { label: "Cyber-Physical Systems", href: "/projects" },
  ],
  quickLinks: [
...
export default function Footer() {
  ...
  return (
    <footer className="relative border-t border-oxford-800 bg-oxford-950">
      <div className="container-xl py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 mb-12">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded bg-white flex items-center justify-center shadow-lg">
                <Cpu className="w-6 h-6 text-oxford-800" />
              </div>
            ...
            <p className="text-gray-400 text-sm leading-relaxed mb-6 max-w-xs">
              Cyber Physical Laboratory — Advancing research in Application Development,
              Machine Learning, Blockchain, IoT, and emerging Cyber-Physical Systems.
            </p>
            {/* Contact info */}
            <div className="space-y-3 mb-8">
              <a href="mailto:research.cplab@gmail.com" className="flex items-center gap-3 text-sm font-medium text-gray-300 hover:text-white transition-colors">
                <Mail className="w-4 h-4 text-amber-400" /> research.cplab@gmail.com
              </a>
              <span className="flex items-center gap-3 text-sm font-medium text-gray-300">
                <MapPin className="w-4 h-4 text-amber-400" /> Basherhat, Dinajpur - 5200, Bangladesh
              </span>
              <a href="tel:+8801712534968" className="flex items-center gap-3 text-sm font-medium text-gray-300 hover:text-white transition-colors">
                <Phone className="w-4 h-4 text-amber-400" /> 01712-534968
              </a>
            </div>
            {/* Social */}
            <div className="flex gap-4">
              {socialLinks.map(({ icon: Icon, href, label }) => (
                <a
                  ...
                  aria-label={label}
                  className="w-10 h-10 rounded bg-oxford-800 flex items-center justify-center text-gray-300 hover:text-white hover:bg-oxford-700 transition-colors"
                >
                  <Icon />
                </a>
              ))}
          ...
          {/* Navigation */}
          <div>
            <h3 className="font-bold text-white text-sm uppercase tracking-wider mb-5">Navigation</h3>
          ...
          {/* Research Areas */}
          <div>
            <h3 className="font-bold text-white text-sm uppercase tracking-wider mb-5">Research</h3>
            <ul className="space-y-3">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {footerLinks.quickLinks.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-sm font-medium text-gray-400 hover:text-white transition-colors flex items-center gap-2 group">
                    {link.label}
